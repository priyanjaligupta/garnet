...
namespace Garnet.cluster
{
    ...
    internal sealed partial class ClusterManager : IDisposable
    {
        ...
        /// <returns>True on success, false otherwise</returns>
        public bool TryPrepareSlotForOwnershipChange(int slot, string nodeid, string uid, out ReadOnlySpan<byte> errorMesage)
        {
            errorMesage = default;
            var current = currentConfig;
            ...
            else if (current.GetState((ushort)slot) is SlotState.IMPORTING)
            {
                ...
                while (true)
                {
                    ...
                    if (Interlocked.CompareExchange(ref currentConfig, newConfig, current) == current){
                        ...
                        var newNodeView= currentConfig.GetShardsInfo();
                        var oldNodeView=current.GetShardsInfo();
                        logger?.LogTrace($"[DEBUGGING]  Timestamp: {timestamp} UID: {uid}  newConfig nodeView: {newNodeView} oldConfig nodeView: {oldNodeView}");
                        break;
                    }
                }
                FlushConfig();
